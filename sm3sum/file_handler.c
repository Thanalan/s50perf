...
void read_and_calc(FILE *fp, size_t file_size) {
    ...
    while (calculated + (BLOCK_BATCH_CNT + 1)* BLOCK_SIZE <= file_size) {
        ...
        read_succ = fread(buf, BLOCK_BATCH_CNT * BLOCK_SIZE , 1, fp);
        assert(read_succ);
        // does not padding
        pacc_sm3_update(&ctx, buf, BLOCK_BATCH_CNT * BLOCK_SIZE );
    ...
    pacc_sm3_update(&ctx, buf, final_size );
...
void stdin_read_and_calc() {
    ...
    while (c != EOF) {
        ...
        ++offset;
        c = getchar();
        if (offset == BLOCK_SIZE && c != EOF) {
            offset = 0;
            // ready to calculate
            //sm3_iterate(buf, BLOCK_SIZE);
             pacc_sm3_update(&ctx, buf, BLOCK_SIZE);
        }
    }
    ...
    pacc_sm3_update(&ctx, buf, offset);
	pacc_sm3_final(&ctx, V);
    free(buf);
}
